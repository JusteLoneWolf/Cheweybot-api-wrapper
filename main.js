...
function start() {
    if(data.running === false){
        ...
        console.log('[CheWeyBot-Wrapper] [Analytics] Analystics is already started')
    }
}
function stop() {
        if(data.running === true){
            ...
            console.log('[CheWeyBot-Wrapper] [Analytics] Analystics is not started')
        }
}
    ...
    return new Promise(async (resolve) => {
        if (data.lib != null) {
            if (data.lib.user === null) {
                data.lib.on("ready", () => {
                    console.log('[CheWeyBot-Wrapper] [Analytics] Ready');
                    autoloader();
...
function autoloader() {
        if (data.lib && !data.interval) {
            ...
            console.log('[CheWeyBot-Wrapper] [Analytics] Autoload has been started')
        } else {
            console.log('[CheWeyBot-Wrapper] [Analytics] Analystics is not initialized')
        }
}
...
function stopautoloader() {
    if (data.interval) {
        ...
        console.log('[CheWeyBot-Wrapper] [Analytics] Autoload has been stop')
    } else {
        console.log('[CheWeyBot-Wrapper] [Analytics] Autoload is not initialized')
    }
}

function MakeData() {
    console.log('[CheWeyBot-Wrapper] [Analytics] Collecting Data...')
...
function sender(send) {
    return new Promise(async (resolve, reject) => {
        ...
        console.log('[CheWeyBot-Wrapper] [Analytics] Send Data...');
        axios.post('https://api.chewey-bot.ga/analytics/post', content, {
            ...
            console.log(`[CheWeyBot-Wrapper] [Analytics] Post done !\n${res.data.url}`);
        }).catch(err => {
            if (settings.token) {
                if (err.response.status === 403) {
                    return console.error(new error.IncorrectLogin('Incorrect token provided'))
                } else if (err.response.status === 429) {
                    return console.error('[CheWeyBot-Wrapper] [Analytics] Error: Too many request HTTPError: 429')
                }
            }
            reject(`[CheWeyBot-Wrapper] [Analytics] ERROR: ${err}`)
        })
    })
}